...
app.controller('indexController', ['$scope', '$http', '$cookies', '$window', '$rootScope', 'Upload',  function($scope, $http, $cookies, $window,  $rootScope, Upload) {
	...
	//Informatiques
	var fieldsI = [{name:'I1', niveau:'', duree:''}];
	$scope.formDataI = {};
	$scope.formDataI.dynamicFields = fieldsI;

	//Habilitations
	var fieldsH = [{name:'H1', obtention:'', expire:''}];
	$scope.formDataH = {};
	$scope.formDataH.dynamicFields = fieldsH;

	//Expérience
	//var fieldsE = [{name:'E1', fonction:'', depuis:'', fin:'', remun:'', interess:'', avantage:'', motifDepart:'', vehFonc:'', vehServ:'', indemnDepl:'', repas:'', prime:'', autrePrime:'', problem:'', objectif:'', activite:'', resultat:'', dernierJob:'', nomSoc:'', lieuSoc:'', caSoc:'', nbSalSoc:'', nomContact:'', prenomContact:'', fctContact:'', numContact:''}];
	var fieldsE = [{name:'E1', fonction:'', depuis:'', fin:''}];
	$scope.formDataE = {};
	$scope.formDataE.dynamicFields = fieldsE;
	//$scope.formDataE.dynamicFields.idDebut = 0
	...
	$scope.addE=function(){
		...
		$scope.formDataE.dynamicFields.push( {name: 'E'+newENum, fonction:'', depuis:'', fin:''});
	}

	$scope.remE=function(id){
...
/**************************************************************/
		for(i=0;i<$scope.formDataT.dynamicFields.length;i++){
			$http({
	  			...
	  			datatype: 'json',
	  			data: {	'code_p': $scope.userInfos.code_p,
						'code_c': $scope.formDataT.dynamicFields[i],
...
/**************************************************/
		...
		for(i=0;i<$scope.formDataE.dynamicFields.length;i++){
			$http({
	  			url: '/experience/addExperience',
	  			method: 'POST',
	  			datatype: 'json',
	  			data: {	'code_p': $scope.userInfos.code_p,
						'fonction': $scope.formDataE.dynamicFields[i]['fonction'],
						'depuis': $scope.formDataE.dynamicFields[i]['depuis'],
						'fin': $scope.formDataE.dynamicFields[i]['fin'],
						'remun': $scope.formDataE.dynamicFields[i]['remun'],
						'interess': $scope.formDataE.dynamicFields[i]['interess'],
						'avantage': $scope.formDataE.dynamicFields[i]['avantage'],
						'motifDepart': $scope.formDataE.dynamicFields[i]['motifDepart'],
						'vehFonc': $scope.formDataE.dynamicFields[i]['vehFonc'],
						'vehServ': $scope.formDataE.dynamicFields[i]['vehServ'],
						'indemnDepl': $scope.formDataE.dynamicFields[i]['indemnDepl'],
						'repas': $scope.formDataE.dynamicFields[i]['repas'],
						'prime': $scope.formDataE.dynamicFields[i]['prime'],
						'autrePrime': $scope.formDataE.dynamicFields[i]['autrePrime'],
						'problem': $scope.formDataE.dynamicFields[i]['problem'],
						'objectif': $scope.formDataE.dynamicFields[i]['objectif'],
						'fonction': $scope.formDataE.dynamicFields[i]['fonction'],
						'activite': $scope.formDataE.dynamicFields[i]['activite'],
						'resultat': $scope.formDataE.dynamicFields[i]['resultat'],
						'dernierJob': $scope.formDataE.dynamicFields[i]['dernierJob'],
						'nomSoc': $scope.formDataE.dynamicFields[i]['nomSoc'],
						'lieuSoc': $scope.formDataE.dynamicFields[i]['lieuSoc'],
						'caSoc': $scope.formDataE.dynamicFields[i]['caSoc'],
						'nbSalSoc': $scope.formDataE.dynamicFields[i]['nbSalSoc'],
						'nomContact': $scope.formDataE.dynamicFields[i]['nomContact'],
						'prenomContact': $scope.formDataE.dynamicFields[i]['prenomContact'],
						'fctContact': $scope.formDataE.dynamicFields[i]['fctContact'],
						'numContact': $scope.formDataE.dynamicFields[i]['numContact'],
						'permContact': $scope.formDataE.dynamicFields[i]['permContact']
					},
	  			contentType: 'text/plain',
	  			headers: {'Content-Type': 'application/json'}
	  		})
